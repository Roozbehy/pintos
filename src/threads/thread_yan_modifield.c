...
*/
/*<new version>*
old version is just simply add thread to the tail of the readylist , it ignored the priorty , whatever priority is it will be add to the tail of the list . 
new version I made is , the first element of readyList has the highest priority.
definition of list_insert_orderd:
list_insert_orderd (struct list *list , struct list_elem *elem, list_less_func *less, void *aux
pri_more is the function to compare  
*/
...
/*my Aux function 
FUNCTION: compare 2 different threads , and return true if fst thread have higher priority*/
bool
...
thread_set_priority_other (struct thread *curr, int new_priority, bool forced ){
	if (!curr->donated) //if current thread is donated ,i.e , the priority of current thread is someone else donate to it . the field "donated" is not been implemented before , I've implemented it in file thread.h
		curr->priority = curr->old_priority = new_priority;

	else if (forced) {
		if (current->priority > new_priority)  
			curr->old_priority = new_priority; //priority_change will fail if this sentence not written
		else 
			curr->priority = new_priority;
		}
	else curr-priority = new_priority;

	if(list_entry(list_begin (&ready_list),struct thread, elem) -> priority >= new_priority){ //if current thread no longer has the highest priority 
		thread_yield (); // we yield this thread 
	}
